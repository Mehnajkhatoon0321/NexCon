...
class DelegateRegister extends StatefulWidget {
  ...
  const DelegateRegister(
      {required this.title, required this.selectedRole, super.key});
...
class _DelegateRegisterState extends State<DelegateRegister> {
  ...
  String? selectStateNamesCategories;
  ...
  static const List<String> stateNamesCategories = [
    'Uttar Pradesh',
    'Maharashtra',
    'Delhi',
    'Gujarat',
  ...
  Widget build(BuildContext context) {
    ...
    return MediaQuery(
      ...
      child: Scaffold(
        ...
        body:
          BlocListener<AuthFlowBloc, AuthFlowState>(
            listener: (context, state) {
              if (state is AuthFlowLoading) {
                setState(() {
                  isLoading = true;
                });
              } else if (state is DelegatesRegisterSuccess) {
                setState(() {
                  isLoading = false;
                  var responseData = state.delegatesRegisterSuccess;
                });
              } else if (state is DelegatesRegisterFailure) {
                setState(() {
                  isLoading = false;
...
// State Label + Dropdown
                        ...
                        Padding(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            value: selectStateNamesCategories,
                            hint: const Text(
                              "Select state",
                              style: FTextStyle.formhintTxtStyle,
                            ),
                            items: stateNamesCategories
                            ...
                            onChanged: (newValue) {
                              setState(() {
                                selectStateNamesCategories = newValue;
                              });
                                    ...
                                    setState(() {
                                      if (formKey.currentState!.validate()) {
                                        RegisterDelegateEventHandler(
                                          ...
                                          delegateCategoryId: int.tryParse(selectCategoryName ?? '1') ?? 1,
                                          mobile: _mobileNumber.text.trim(),
                                          email: _email.text.trim(),
                                          dob: _dateBirth.text.trim(), // Expected in format: dd-MM-yyyy
                                          gender: genderTitleName ?? '',
                                          password: _password.text.trim(),
                                          passwordConfirmation: _password.text.trim(), // Or confirm password if you have another field
                                          roleId: 1,
                                          country: selectCountryNamesCategories ?? '',
                                          state: selectStateNamesCategories ?? '',
                                          city: _city.text.trim(),
                                        );

                                      // BlocProvider.of<AuthFlowBloc>(context).add(
                                        //   RegisterDelegateEventHandler(conferenceId: 1, title: '', name: '', lastName: '',
                                        //       delegateCategoryId: 2, mobile: '', email: '', dob: '',
                                        //       gender: '', password: '', roleId: 1, passwordConfirmation: '', country: '', state: '', city: ''
                                        //
                                        //
                                        //   ),
                                        // );
