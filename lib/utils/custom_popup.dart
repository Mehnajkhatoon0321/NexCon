...
class CustomPopUp {
          ...
          data: ThemeData(
            ...
            textTheme: TextTheme(
              ...
              titleLarge: TextStyle(
                  color: AppColors.appSky, fontSize: 20), // Header text style
            ),
            iconTheme: IconThemeData(color: AppColors.appSky),
            colorScheme: ColorScheme.fromSwatch().copyWith(
                secondary: AppColors.appSky), // Icon color (e.g., arrows)
          ),
  ...
  static Future<void> selectDateAndTime(BuildContext context,
      TextEditingController dateController) async {
    DateTime currentDate = DateTime.now();
    DateTime? selectedDate = await showDatePicker(
      context: context,
    ...
    if (selectedDate != null) {
      ...
      if (selectedTime != null) {
        ...
        String formattedDateTime = DateFormat('yyyy-MM-dd hh:mm a').format(
            selectedDateTime);
  ...
  static Future<void> datePicker(BuildContext context,
      TextEditingController dateController) async {
    DateTime currentDate = DateTime.now();
      ...
      builder: (BuildContext context, Widget? child) {
        return Theme(
          data: ThemeData(
            ...
            iconTheme: IconThemeData(color: AppColors.appSky),
            colorScheme: ColorScheme.fromSwatch().copyWith(
                secondary: AppColors.appSky), // Icon colors
          ),
          child: child ?? Container(),
        );
      },
